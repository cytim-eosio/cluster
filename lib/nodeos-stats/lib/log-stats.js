...
const setMetrics = (ctx, type, metrics) => {
  const metricset = ctx[type];
  metricset.count++;
  ...
  return (line) => {
    ...
    if (matches) {
      ...
      if (!(options.skipZeroTpb && tpb === 0)) {
        setMetrics(ctx, 'produced', { tpb });
      }
    }

    metches = line.match(/Received block.+\[trxs: (\d+).+, latency: (\d+)/);
    if (matches) {
      const tpb = parseInt(matches[1]);
      const latency = parseInt(matches[2]);
      if (!(options.skipZeroTpb && tpb === 0)) {
        setMetrics(ctx, 'received', { tpb, latency });
      }
    }
